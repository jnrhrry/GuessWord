...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val quizData = Data.getData.toMutableList()
        quizData.shuffle()
        ...
        btnAnswerCheck.setOnClickListener {
            answerCounter++
            if (answer == correctAnswer.toUpperCase()) {
                ...
                if (quizCounter == 10) {
                    ...
                    Snackbar.make(rootView, "Tepat Sekali!", Snackbar.LENGTH_LONG).show()
                    remove(correctAnswer)
                    generateQuiz(quizData[quizCounter])
                }

            } else {
                Snackbar.make(rootView, "Hampir Benar!", Snackbar.LENGTH_LONG).show()
                reset(correctAnswer)
            }
        }
    }
    ...
    private fun reset(word: String) {
        ...
        btnAnswerCheck.visibility = View.GONE
    ...
    private fun generateQuiz(quiz: Quiz) {
    ...
    private fun letterClickListener(word: String, index: Int) = View.OnClickListener {
        ...
        counter++
        when {
            counter >= word.length + 1 -> btnAnswerCheck.visibility = View.VISIBLE
